...
### 3.16 Solace PubSub+ Constructs

**Package**: `@cdk8s-charts/solace`

Wraps the three upstream [Solace PubSub+ Software Event Broker](https://dev.solace.com)
Helm charts. The Solace advanced event broker streams events over AMQP, JMS,
MQTT, REST, SMF, and WebSocket; brokers in multiple locations can be linked
into a Solace event mesh to dynamically share events across cloud, on-prem,
and hybrid environments.

The three charts share an identical values schema (only defaults differ), so
the package exposes one typed `SolaceValues` model, one `SolaceProps`, one
`SolaceExports`, and a shared `SolaceBase` class. Three thin constructs pin
per-chart defaults:

| Construct | Chart | Topology | Default `redundancy` | Default `size` | Default `storage.size` |
|-----------|-------|----------|----------------------|----------------|------------------------|
| `PubsubPlus` | `pubsubplus` | single-node non-HA | `false` | `prod1k` | `30Gi` |
| `PubsubPlusDev` | `pubsubplus-dev` | minimum footprint single-node (dev) | `false` | `dev` | `10Gi` |
| `PubsubPlusHa` | `pubsubplus-ha` | HA redundancy group (Primary/Backup/Monitor) | `true` | `prod1k` | `30Gi` |

**Chart repo:** `https://solaceproducts.github.io/pubsubplus-kubernetes-helm-quickstart/helm-charts/`
(non-OCI, uses `repo`). Default version pin: `3.10.0`.

| Prop | Type | Required | Purpose |
|------|------|----------|---------|
| `namespace` | `string` | yes | K8s namespace |
| `redundancy` | `boolean` | no | HA (Primary/Backup/Monitor); defaults per chart |
| `size` | `'dev' \| 'prod1k' \| 'prod10k' \| 'prod100k'` | no | Scaling tier; defaults per chart. Ignored when `systemScaling` is set |
| `systemScaling` | `SolaceSystemScaling` | no | Fine-grained scaling; overrides `size` when provided |
| `adminPassword` | `string` | no | Plain-text admin password (autogenerated if omitted; not recommended for production) |
| `adminPasswordSecretName` | `string` | no | Existing Secret (key `username_admin_password`) holding the admin password; takes precedence over `adminPassword` |
| `timezone` | `string` | no | tz database timezone name; default UTC |
| `tls` | `SolaceTlsProps` | no | Convenience TLS toggle (`enabled`, `serverCertificatesSecret`, cert/key filenames) |
| `serviceType` | `'ClusterIP' \| 'NodePort' \| 'LoadBalancer'` | no | Service exposure; default `LoadBalancer` |
| `storage` | `SolaceStorageProps` | no | Convenience storage knobs (`persistent`, `size`, `useStorageClass`, `slow`, `useStorageGroup`, `monitorStorageSize`, `customVolumeMount`) |
| `insights` | `SolaceInsightsProps` | no | Solace Insights agent toggle + API key/site/tags + forwarding config |
| `chart` | `string` | no | Helm chart name/ref; defaults to the wrapped chart |
| `repo` | `string` | no | Helm repository URL; defaults to the upstream Solace charts repo |
| `version` | `string` | no | Helm chart version pin; defaults to `3.10.0` |
| `values` | `DeepPartial<SolaceValues>` | no | Raw Helm value overrides (deep-merged into computed values) |

| Export | Type | Value |
|--------|------|-------|
| `host` | `string` | Service DNS name (`{id}`) |
...
1. Each construct renders its upstream Solace Helm chart with the shared Solace
   chart repository flag (`--repo`) and version pin.
2. `SolaceBase` maps convenience props (`redundancy`, `size`, `systemScaling`,
   `adminPassword`, `adminPasswordSecretName`, `timezone`, `tls`, `serviceType`,
   `storage`, `insights`) into the chart's `solace.*`, `tls.*`, `service.*`,
   `storage.*`, and `insights.*` value sections.
3. Per-chart defaults are applied when the corresponding prop is omitted
   (e.g. `PubsubPlusHa` defaults `redundancy=true`; `PubsubPlusDev` defaults
   `size=dev` and `storage.size=10Gi`).
...
6. **Paseo auto-resume** — preStop hook snapshots open (non-closed,
   ...
   shrink the set). Snapshot status decides dispatch: `paseo send` a
   "continue" prompt to agents that were mid-turn, `paseo agent reload`
   quiet ones (reattach runtime, no spurious turn); legacy status-less
   entries count as mid-turn. Mid-turn agents run first so the
   `PASEO_AUTO_RESUME_MAX` cap drops warm-up reloads, not prompts.
