...
cdk8s-charts/
  packages/
    ...
    charts/
      ...
      pubsubplus/                   @cdk8s-charts/pubsubplus
        src/types.ts                Solace PubSub+ Helm values + Props/Exports
        src/construct.ts            PubsubPlus construct (event broker)
...
### 3.16 PubSubPlus Construct

**Package**: `@cdk8s-charts/pubsubplus`

Wraps the [Solace PubSub+ Software Event Broker](https://dev.solace.com)
Helm chart — a single-node (non-HA) deployment of the Solace advanced event
broker that streams events over AMQP, JMS, MQTT, REST, SMF, and WebSocket. The
broker can be linked into a Solace event mesh to dynamically share events
across cloud, on-prem, and hybrid environments.

**Chart:** `pubsubplus` from
...
|------|------|----------|---------|
| `namespace` | `string` | yes | K8s namespace |
| `redundancy` | `boolean` | no | `true` for HA (Primary/Backup/Monitor); default `false` (single-node) |
| `size` | `'dev' \| 'prod1k' \| 'prod10k' \| 'prod100k'` | no | Scaling tier by max connections; default `prod1k`. Ignored when `systemScaling` is set |
| `systemScaling` | `PubsubPlusSystemScaling` | no | Fine-grained scaling; overrides `size` when provided |
...
| `tls` | `PubsubPlusTlsProps` | no | Convenience TLS toggle (`enabled`, `serverCertificatesSecret`, cert/key filenames) |
| `serviceType` | `'ClusterIP' \| 'NodePort' \| 'LoadBalancer'` | no | Service exposure; default `LoadBalancer` |
| `storage` | `PubsubPlusStorageProps` | no | Convenience storage knobs (`persistent`, `size`, `useStorageClass`, `slow`, `useStorageGroup`, `monitorStorageSize`, `customVolumeMount`) |
| `insights` | `PubsubPlusInsightsProps` | no | Solace Insights agent toggle + API key/site/tags + forwarding config |
...
| `values` | `DeepPartial<PubsubPlusValues>` | no | Raw Helm value overrides (deep-merged into computed values) |
...
1. Renders the upstream Solace `pubsubplus` Helm chart with the Solace chart
   repository flag (`--repo`).
2. Maps convenience props (`redundancy`, `size`, `systemScaling`, `adminPassword`,
   `adminPasswordSecretName`, `timezone`, `tls`, `serviceType`, `storage`,
   `insights`) into the chart's `solace.*`, `tls.*`, `service.*`, `storage.*`,
   and `insights.*` value sections.
3. Defaults to single-node (`solace.redundancy=false`), `prod1k` scaling tier,
   `LoadBalancer` service, and persistent storage (`30Gi`).
