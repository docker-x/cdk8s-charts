...
export class Devenv extends HelmConstruct<Values> {
  ...
  private createSecrets(
    ...
    name: string,
    namespace: string,
    d: ReturnType<Devenv['deriveState']>,
  ) {
  ...
  private buildContainerEnv(values: Values, name: string) {
    ...
    // Devenv-specific env vars
    env.push({ name: 'DEVENV', value: 'true' });
    seen.add('DEVENV');
    if (values.env)
  ...
  private createDeployment(opts: {
    ...
    values: Values;
    d: ReturnType<Devenv['deriveState']>;
    containerEnv: ReturnType<Devenv['buildContainerEnv']>;
    volumeMounts: ReturnType<Devenv['buildVolumeMounts']>;
    volumes: ReturnType<Devenv['buildVolumes']>;
    props: Props;
  }) {
